...
description: Build a new OpenCLI site adapter or command, with source-contract selection and output verification.
...
数据可见且属于 HTTP/JSON/HTML 时继续；实时推送需求只有找到合适的 HTTP 替代路径时才继续。
...
确定 strategy 和证据后，完成适配器实现、相关验证和结果核对。
需要探索步骤、字段解码、验收清单或降级路径时，按需读取
[authoring workflow](references/authoring-workflow.md)。只读与写入的合同门禁仍适用。
