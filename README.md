...
> Store. The current lines are CLI `2.0.4` (`kyangc-v2.0.4`) and extension
> `2.0.0` (`kyangc-ext-v2.0.0`), based on upstream `1.8.8` / extension
> `1.0.24`.
...
```bash
node --version
grep 'jackwener-opencli-2.0.4.tgz$' SHA256SUMS | sha256sum -c -
# macOS: use `shasum -a 256 -c -` after the pipe instead

OPENCLI_RELEASE=kyangc-v2.0.4
OPENCLI_PREFIX="$HOME/.local/share/opencli/releases/$OPENCLI_RELEASE/runtime"
npm install --prefix "$OPENCLI_PREFIX" --global ./jackwener-opencli-2.0.4.tgz
