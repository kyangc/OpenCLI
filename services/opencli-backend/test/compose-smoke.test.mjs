...
`);
  ...
  try {
    await execFileAsync('/bin/sh', [composeSmoke, '2.0.4', '2.0.0'], {
      env: {
        ...process.env,
    ...
    assert.match(calls, /node .*smoke-deployment\.mjs --base-url http:\/\/127\.0\.0\.1:28080 --expected-daemon-version 2\.0\.4 --expected-extension-version 2\.0\.0/);
...
`);
  ...
  try {
    await assert.rejects(
      execFileAsync('/bin/sh', [composeSmoke, '2.0.4', '2.0.0'], {
        env: {
          ...process.env,
          PATH: `${fakeBin}:${process.env.PATH}`,
          SMOKE_CALL_LOG: callLog,
